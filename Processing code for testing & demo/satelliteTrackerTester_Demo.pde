...
float zeroToCenter(float num, float axisLen) {
  return num - (axisLen / 2);
}

float centerToZero(float num, float axisLen) {
  return num + (axisLen / 2);
}

float zeroDegRightToUp(float a) {
  return a + 90;
}

float zeroDegUpToRight(float a) {
  return a - 90;
}
...
String arrToString(String strIn[]) {
  String strOut = "";
